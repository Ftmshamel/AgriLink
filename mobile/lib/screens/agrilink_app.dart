...
void openUserHome(BuildContext context, MobileUser user) {
  Navigator.pushAndRemoveUntil(
    ...
    MaterialPageRoute(
      builder: (_) {
        if (user.verification['status'] == 'pending_review') {
          return VerificationPendingPage(user: user);
        }
        return switch (user.role) {
          MobileRole.consumer => ConsumerShell(user: user),
          MobileRole.rider => RiderShell(user: user),
          MobileRole.farmer ||
